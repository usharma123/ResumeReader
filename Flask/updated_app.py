...
def extract_skills_with_model(text, skills_to_look_for):
    skills = []
    for skill in skills_to_look_for:
        if skill in text:
            skills.append(skill)
    return skills
...
def upload_file():
    if request.method == 'POST':
        ...
        for file in uploaded_files:
            ...
            skills = extract_skills_with_model(processed_text, skills_list)
...
def llm_intro():
    if request.method == 'POST':
        ...
        # Extract skills from the LLM response
        new_skills = extract_skills_from_response(response)
        
        return jsonify({"conversation": conversation, "response": response, "skills": new_skills})
    else:
        return render_template('llm_intro.html')

def extract_skills_from_response(response):
    # Extracting skills mentioned in the LLM response
    response_text = response.lower()
    extracted_skills = [skill for skill in skills_list if skill in response_text]
    return extracted_skills

@app.route('/llm_intro_upload', methods=['POST'])
def llm_intro_upload():
    ...
    # Extract skills from the LLM response
    new_skills = extract_skills_from_response(response)
    # Extract skills from the resume using the new skills
    skills = extract_skills_with_model(processed_text, new_skills)
    
    return jsonify({"conversation": conversation, "response": response, "skills": skills})
